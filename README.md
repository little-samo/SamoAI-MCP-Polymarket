...
| `redeem_positions` | Claim one or more winning positions after market resolution, with gasless or on-chain execution |
...
If `POLY_BUILDER_API_KEY`, `POLY_BUILDER_SECRET`, and `POLY_BUILDER_PASSPHRASE` are all set, `redeem_positions` will try a gasless claim through Polymarket's Builder Relayer first. If the gasless flow fails, the server automatically falls back to the normal on-chain transaction path. You can pass either a single `condition_id` or multiple `condition_ids` to redeem several resolved markets in one tool call.
